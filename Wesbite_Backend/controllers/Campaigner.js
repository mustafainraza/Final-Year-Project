...
const {
  campaignExtensionRequest,
} = require("../services/Campaigner/campaign_extension_request");
const {
  campaignerchangepassword,
} = require("../services/Campaigner/campaigner_changepassword");
const { campaignerlogin } = require("../services/Campaigner/campaigner_login");
const {
  campaignerresetpassword,
} = require("../services/Campaigner/campaigner_resetpassword");
const {
  campaignersignup,
} = require("../services/Campaigner/campaigner_signup");
const {
  campaignersubmitchangepassword,
} = require("../services/Campaigner/campaigner_submitchangepassword");
const { createcampaign } = require("../services/Campaigner/create_campaign");
const {
  getCampaignDetails,
} = require("../services/Campaigner/get_campaign_details");
...
module.exports.create_campaign = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .send("Campaign Created Request Sent Successfully to Admin.");
  } catch (error) {
    return res.status(500).json({ msg: `${error.message}` });
  }
};
...
module.exports.campaign_extension_request = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .send("Campaign Time Extend Request Sent Successfully");
  } catch (error) {
    return res.status(401).json({ msg: error.message });
  }
};
